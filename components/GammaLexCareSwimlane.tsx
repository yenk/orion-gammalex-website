...
export default function GammaLexInteractiveStepper() {
  ...
  return (
    <section className="w-full flex flex-col items-center py-8 sm:py-16 px-2 sm:px-0 bg-gradient-to-b from-sage-50/80 to-white">
      <h2 className="text-2xl sm:text-3xl lg:text-4xl font-inter font-normal text-center text-slate-900 mb-8 sm:mb-12 max-w-2xl mx-auto px-4">
      ...
      <div className="w-full max-w-6xl mx-auto px-4 sm:px-0">
        ...
        <div className="flex justify-center mb-6 sm:mb-8">
          <div className="flex flex-wrap justify-center items-center gap-1 sm:gap-2 bg-white rounded-2xl p-2 sm:p-3 shadow-lg border border-sage-100 max-w-full">
            {steps.map((step, index) => (
              <button
                key={step.id}
                onClick={() => goToStep(step.id)}
                className={`flex items-center gap-1 sm:gap-2 px-2 sm:px-4 py-2 rounded-xl transition-all duration-200 min-w-0 ${
                  currentStep === step.id
              ...
              >
                ...
                <span className="hidden sm:inline text-xs sm:text-sm truncate max-w-20 sm:max-w-none">{step.title}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="flex justify-center mb-6 sm:mb-8">
          ...
          >
            <div className={`w-2 h-2 sm:w-3 sm:h-3 rounded-full ${isAutoPlay ? 'bg-white' : 'bg-gammalex-orange'}`} />
            <span className="hidden sm:inline">{isAutoPlay ? 'Auto-play On' : 'Auto-play Off'}</span>
            <span className="sm:hidden">{isAutoPlay ? 'Auto' : 'Manual'}</span>
          </button>
        </div>

        {/* Main Content - Mobile Optimized */}
        <div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl p-4 sm:p-8 border border-sage-100">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="flex flex-col items-center lg:items-start gap-4 sm:gap-6">
                <div 
                  className="w-16 h-16 sm:w-20 sm:h-20 lg:w-24 lg:h-24 rounded-xl sm:rounded-2xl flex items-center justify-center text-white shadow-lg"
